...
export class Indexer implements IndexerInterface {
  ...
  async loadSubgraphEntityRelations<Entity> (entity: new () => Entity, blockHash: string, data: any): Promise<any> {
    ...
    for (const field in relations) {
      ...
      data[field] = await this.getSubgraphEntity(relatedEntity, id, blockHash);

      data[field] = this.loadSubgraphEntityRelations(relatedEntity, blockHash, data[field]);
    }
  ...
  _populateRelationsMap (): void {
    ...
    this._relationsMap.set(ExampleEntity, { related: RelatedEntity });
  }
