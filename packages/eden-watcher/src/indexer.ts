...
export class Indexer implements IndexerInterface {
  ...
  async getSubgraphEntity<Entity> (entity: new () => Entity, id: string, blockHash: string): Promise<any> {
    const data = await this._graphWatcher.getEntity(entity, id, blockHash);

    return this._loadSubgraphEntityRelations(entity, blockHash, data);
  }
  ...
  async _loadSubgraphEntityRelations<Entity> (entity: new () => Entity, blockHash: string, data: any): Promise<any> {
    const relations = this._relationsMap.get(entity);

    for (const field in relations) {
      const idOrIds = data[field];
      const relatedEntity = relations[field];

      // TODO: Results in endless loop if there is a cyclic relation in schema (without derivedFrom).
      if (Array.isArray(idOrIds)) {
        const dataPromises = idOrIds.map(async id => {
          const relatedData = await this._graphWatcher.getEntity(relatedEntity, id, blockHash);

          return this._loadSubgraphEntityRelations(relatedEntity, blockHash, relatedData);
        });

        data[field] = await Promise.all(dataPromises);
      } else {
        data[field] = await this._graphWatcher.getEntity(relatedEntity, idOrIds, blockHash);

        data[field] = await this._loadSubgraphEntityRelations(relatedEntity, blockHash, data[field]);
      }
    }
  ...
  _populateRelationsMap (): void {
    ...
    this._relationsMap.set(ProducerSet, { producers: Producer });
    ...
    this._relationsMap.set(ProducerEpoch, { epoch: Epoch });
    ...
    this._relationsMap.set(SlotClaim, { slot: Slot });

    this._relationsMap.set(Network, { stakers: Staker });

    this._relationsMap.set(Distributor, { currentDistribution: Distribution });

    this._relationsMap.set(Distribution, { distributor: Distributor });

    this._relationsMap.set(Claim, { account: Account });

    this._relationsMap.set(Slash, { account: Account });
  }
